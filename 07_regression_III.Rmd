...
```{r}
fit_2 <- glm(log_viol_r ~ largest50, data=df)
```
...
Looking only at covariation between pairs of variables can be misleading. It may lead you to conclude that a relationship is more important than it really is. This is no trivial matter, but one of the most important ones we confront in research and policy[^11]. 
...
It could not be any easier to fit a multiple regression model. You simply modify the formula in the `glm()` function by adding terms for the additional inputs.

```{r}
fit_3 <- glm(log_viol_r ~ unemployed + largest50, data=df)
summary(fit_3)
```
...
```{r}
fit_4 <- glm(log_viol_r ~ unemployed + largest50 + black + fborn + log_incarceraton, data=df)
plot_model(fit_4)
```
...
```{r}
fit_5 <- glm(log_viol_r ~ unemployed + largest50 + unemployed:largest50 , data=df)
# which is equivalent to: 
# fit_5 <- glm(log_viol_r ~ unemployed * largest50, data=BCS0708)
summary(fit_5)
```
