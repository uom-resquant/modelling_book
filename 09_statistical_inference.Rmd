...
+ First, under "sample estimates", we have the expression "mean of x" indicating sample mean of the variable `tcviolent`, which is `r round(mean(csew_0708$tcviolent, na.rm = T), 2)`, as we already knew. 
+ Above that, under "95 percent confidence interval", we have two numbers. They the lower bound and the upper bound of the 95% confidence interval, respectively. Therefore, the 95% confidence interval is $[$ `r round(t.test(csew_0708$tcviolent)$conf.int[1], 2)` ; `r round(t.test(csew_0708$tcviolent)$conf.int[2], 2)` $]$.
...
The sample percentage of individuals who have been victimised is `r paste(round(prop.table(table(csew_0708$bcsvictim))[2], 4) * 100, "%", sep = "")`. Yet, this is only true for our sample. The number we actually want to find out is the *population* percentage of individuals who have been victimised. Since this is a population parameters, it is unknown. We can only guess. Our best guess is to use the sample statistic. Therefore, we *estimate* that the population percentage of individuals who have been victimised is `r paste(round(prop.table(table(csew_0708$bcsvictim))[2], 4) * 100, "%", sep = "")`.
