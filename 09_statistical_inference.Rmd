...
So, we can see that this generated variable, which we are treating as the population, ranges from `r min(fake_population)` to `r max(fake_population)`, with a standard deviation of `r round(sd(fake_population), 2)`. The average score is `r round(mean(fake_population), 2)`. Let's remember this number. 
...
`r sum(fake_population > 0)` individuals have committed at least one crime in the previous year. That implies `r paste(round(sum(fake_population > 0) / length(fake_population), 4) * 100, "%", sep="")` of the population. Let's remember this number too.
...
Good. So, we have a population in which `r paste(round(prop.table(table(fake_population$offender))[2], 4) * 100, "%", sep="")` of the individuals have committed at least one offence in the previous year and the average number of offences committed by each individual is `r round(mean(fake_population$crimecount), 2)`.
...
We have a new dataset called `fake_sample_1` in our environment now. It has `r nrow(fake_sample_1)` observations and `r ncol(fake_sample_1)` columns. Do note the `fake_sample_1` object has an additional column named `orig.id`, which indicates the row number of the sampled observation in the original `fake_population` object.

For starters, let's check the average number of offences *sampled* individuals have committed. We expect it to be similar to the *population mean* of `r round(mean(fake_population$crimecount), 2)`.
...
It is almost identical to the population mean! That's great. Now let's check the proportion of individuals who have committed at least one offence. We expect it to be similar to the *population proportion* of `r paste(round(prop.table(table(fake_population$offender))[2], 4) * 100, "%", sep="")`.
...
The standard error was `r round(sd(samples_50k_crimecount$with), 2)`. If we multiply `r round(sd(samples_50k_crimecount$with), 2)` by 1.96, we obtain `r `r round(sd(samples_50k_crimecount$with) * 1.96, 2)`. This means that 95% of the samples in this sampling distribution will have an error that won’t be bigger than that. They will only at most differ from the mean of the sampling distribution by (plus and minus) `r round(sd(samples_50k_crimecount$with) * 1.96, 2)`. However, 5% of the samples will have a margin of error bigger than that (in absolute terms).
...
The mean of `crimecount` in `fake_sample_4` is `r round(mean(fake_sample_4$crimecount), 2)`. In this case, we know that this is slightly different but still fairly close to the population mean of `r round(mean(fake_population$crimcount), 2)`, but in real-life scenarios, we only see the sample mean---never the population mean.

So, we can estimate the confidence interval. Because we want the 95% confidence interval, we define the interval as a range $1.96 *$ the standard error below and above our sample mean of `r round(mean(fake_sample_4$crimecount), 2)`. Given that the standard error in this case is `r round(sd(samples_50k_crimecount$with) * 1.96, 2)`, the lower bound of the 95% confidence interval is simply `r round(mean(fake_sample_4$crimecount), 2)` $- 1.96 *$ `r round(sd(samples_50k_crimecount$with), 2)` $=$ `r round(mean(fake_sample_4$crimecount) - 1.96 * sd(samples_50k_crimecount$with) * 1.96, 2)`, whereas the upper bound of the 95% confidence interval is `r round(mean(fake_sample_4$crimecount), 2)` $+ 1.96 *$ `r round(sd(samples_50k_crimecount$with), 2)` $=$ `r round(mean(fake_sample_4$crimecount) + 1.96 * sd(samples_50k_crimecount$with) * 1.96, 2)`.

What does this mean? Simply that **we are 95% confident that the population mean is within the $[$ `r round(mean(fake_sample_4$crimecount) - 1.96 * sd(samples_50k_crimecount$with) * 1.96, 2)`; `r round(mean(fake_sample_4$crimecount) + 1.96 * sd(samples_50k_crimecount$with) * 1.96, 2)` $]$ range**. More specifically, 95% of all confidence intervals (i.e., from all possible samples, the sampling distribution) will contain the population mean and 5% will miss it.
...
So to reiterate:  
  ...
  + **INCORRECT INTERPRETATION**: “There is a 95% chance that the average number of offences is between `r round(mean(fake_sample_4$crimecount) - 1.96 * sd(samples_50k_crimecount$with) * 1.96, 2)` and `r round(mean(fake_sample_4$crimecount) + 1.96 * sd(samples_50k_crimecount$with) * 1.96, 2)`”. This is a very common misconception! It seems very close to true, but it isn’t because the population mean value is fixed. So, it is either in the interval or not, and you can't possibly know whether that is the case. This is subtle but important.
  ...
  + Is is not terrible to say something like “We are 95% confident that the average number of offences is between `r round(mean(fake_sample_4$crimecount) - 1.96 * sd(samples_50k_crimecount$with) * 1.96, 2)` and `r round(mean(fake_sample_4$crimecount) + 1.96 * sd(samples_50k_crimecount$with) * 1.96, 2)`.” This is a common shorthand for the idea that the calculations “work” 95% of the time. 
...
+ First, under "sample estimates", we have the expression "mean of x" indicating the sample mean of the variable `tcviolent`, which is `r round(mean(csew_0708$tcviolent, na.rm = T), 2)`, as we already knew. 
+ Above that, under "95 percent confidence interval", we have two numbers. They are the lower bound and the upper bound of the 95% confidence interval, respectively. Therefore, the 95% confidence interval is $[$ `r round(t.test(csew_0708$tcviolent)$conf.int[1], 2)` ; `r round(t.test(csew_0708$tcviolent)$conf.int[2], 2)` $]$.

So, we can say that we are 95% confident that the population mean is somewhere within the $[$ `r round(t.test(csew_0708$tcviolent)$conf.int[1], 2)`; `r round(t.test(csew_0708$tcviolent)$conf.int[2], 2)` $]$ range. More accurately, we can conclude that, if we were able to draw several other random samples from the population aged 16 or older in England and Wales, 95% of those samples will yield sample estimates between `r round(t.test(csew_0708$tcviolent)$conf.int[1], 2)` and `r round(t.test(csew_0708$tcviolent)$conf.int[2], 2)`.
...
The 99% confidence level is $[$ `r round(t.test(csew_0708$tcviolent, conf.level = .99)$conf.int[1], 2)` ; `r round(t.test(csew_0708$tcviolent, conf.level = .99)$conf.int[2], 2)` $]$.
...
The sample percentage of individuals who have been victimised is `r paste(round(prop.table(table(csew_0708$bcsvictim))[2], 4) * 100, "%", sep = "")`. Yet, this is only true for our sample. The number we actually want to find out is the *population* percentage of individuals who have been victimised. Since this is a population parameter, it is unknown. We can only guess. Our best guess is to use the sample statistic. Therefore, we *estimate* that the population percentage of individuals who have been victimised is `r paste(round(prop.table(table(csew_0708$bcsvictim))[2], 4) * 100, "%", sep = "")`.
...
Very similar output as before. We have the sample estimate of `r round(prop.table(table(csew_0708$bcsvictim))[2], 3)` and a 95% confidence interval ranging from `r round(prop.test(csew_0708$bcsvictim == "victim of crime")$conf.int[1], 3)` to `r round(prop.test(csew_0708$bcsvictim == "victim of crime")$conf.int[2], 3)`.

So, we can say that we are 95% confident that the population proportion is somewhere within the $[$ `r round(prop.test(csew_0708$bcsvictim == "victim of crime")$conf.int[1], 3)`; `r round(prop.test(csew_0708$bcsvictim == "victim of crime")$conf.int[2], 3)` $]$ range.
...
This index is measured on a scale that ranges from `r round(min(csew_0708$tcarea, na.rm = T), 2)`, indicating people who do not perceive anti-social behaviour in their area at all, to `r round(max(csew_0708$tcarea, na.rm = T), 2)`, indicating people who perceive high levels of anti-social behaviour in their neighbourhood.
